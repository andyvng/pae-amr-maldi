...
def main():
    ...
    result_df.to_csv(os.path.join(
        config["output_dir"], f"result_{antimicrobial}_{model}.csv"), index=False)
    ...
    if config["run_shap"]:  # Make SHAP run optional
        ...
        salient_feature_df.to_csv(os.path.join(
            config["output_dir"], f"SHAP_{antimicrobial}_{model}.csv"), index=False)
